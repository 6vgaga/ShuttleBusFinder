...
- (instancetype)initWithOrigin:(CGPoint)origin andSize:(CGSize)size {
    ...
    self = [self initWithFrame:CGRectMake(origin.x, origin.y, size.width, size.height)];
    if (self) {
        _origin = origin;
...
- (CALayer *)createBgLayerWithColor:(UIColor *)color andPosition:(CGPoint)position {
    ...
    layer.bounds = CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width/self.numOfMenu, self.frame.size.height-1);
